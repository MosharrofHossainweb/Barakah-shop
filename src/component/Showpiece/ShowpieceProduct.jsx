...
import showpieceData from "../../DB/showpieceData"; // <-- তোমাকে এই data বানাতে হবে
import { useThemeContext } from "../../context/ThemeProvider";

const ShowpieceProduct = () => {
  ...
  const applyFilters = () => {
    return showpieceData.filter(item => {
      const matchMaterial = filters.material === "All" || item.material === filters.material;
      ...
      return matchMaterial && matchColor && matchDiscount && matchPrice;
    });
  };
  ...
  const clearFilters = () =>
    setFilters({ material: "All", color: "All", discount: false, priceRange: "All" });
  ...
  return (
    <div className={`${mode === "dark" ? "bg-gray-900" : "bg-gray-100"} container mx-auto py-10`}>
      ...
      <div className="text-center mb-10">
        ...
        >
          Elegant Showpiece Collection
        </h1>
        <p className={`text-lg mt-2 ${textColor}`}>
          Discover unique and decorative showpieces for your home or office.
        </p>
      ...
      <div className={`${cardBg} border shadow-md rounded-2xl p-6 mb-10 max-w-4xl mx-auto`}>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="flex flex-col">
            <label className={`text-sm font-semibold mb-1 ${textColor}`}>Material</label>
            <select
      ...
      <div className="flex flex-wrap justify-center gap-6">
        {filteredData.length > 0 ? (
          filteredData.map((item, index) => (
            <Link key={index} to={`/showpiece/${item.sku}`}>
              <div
              ...
              >
                ...
                <div className="p-4 flex flex-col justify-between flex-grow">
                  ...
                  <ul className={`list-disc list-inside mt-2 text-sm ${cardText}`}>
                    <li>Material: <span className="text-green-500">{item.material}</span></li>
          ...
          <p className={`${textColor} text-lg`}>No showpieces match the filter.</p>
        )}
      </div>
    </div>
  );
...
export default ShowpieceProduct;
